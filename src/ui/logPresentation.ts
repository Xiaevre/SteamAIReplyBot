/**
 * User-Friendly Log Layer - Log Presentation Mapper
 * 将内部结构化事件映射为通俗易懂的“人话日志”
 */

export type UserLogLevel = 'info' | 'warn' | 'error' | 'success';
export type UserLogCategory = 'all' | 'important' | 'error' | 'success' | 'task' | 'steam' | 'send';
...
export function mapLogToPresentation(log: RawLogEntry, index: number = 0): UserLogItem {
  const event = log.event || 'UNKNOWN_EVENT';
  const details = log.details || {};
  const timestamp = log.timestamp || new Date().toISOString();
  const timeStr = timestamp.length >= 19 ? timestamp.substring(11, 19) : timestamp;
  const rawJson = JSON.stringify(log, null, 2);
  const id = `log_${Date.parse(timestamp) || Date.now()}_${index}_${Math.random().toString(36).substring(2, 7)}`;
  ...
  switch (event) {
    ...
    case 'COMMENT_PROCESSING_ERROR': {
      ...
      const isDbReadonly = errStr.includes('readonly database') || errStr.includes('SQLITE_READONLY');
      const isDbError = isDbReadonly || errStr.toLowerCase().includes('database') || errStr.toLowerCase().includes('sqlite');
      ...
      if (isDbReadonly) {
        return {
          ...
          suggestion: '请检查 Bot 数据目录是否具有写入权限或重启程序。',
          isImportant: true,
          technicalEvent: event,
          rawDetails: details,
          rawJson
      ...
      return {
        ...
        suggestion: '点击右侧「查看技术详情」查看完整调用栈。',
        technicalEvent: event,
        rawDetails: details,
        rawJson
      };
    ...
    default: {
      const level: UserLogLevel = log.level === 'ERROR' ? 'error' : (log.level === 'WARN' ? 'warn' : 'info');
      const icon = level === 'error' ? '🔴' : (level === 'warn' ? '🟡' : 'ℹ️');
      const cats: UserLogCategory[] = ['all'];
      if (level === 'error') cats.push('error');
      if (event.includes('SEND')) cats.push('send');
      if (event.includes('SESSION') || event.includes('STEAM')) cats.push('steam');
      if (event.includes('COMMENT') || event.includes('TASK')) cats.push('task');
      ...
      if (typeof details === 'string') detailStr = details;
        ...
        if (keys.length > 0) {
          detailStr = keys.map(k => `${k}: ${details[k]}`).join(' | ');
        }
      }
...
// Attach to browser window if in browser environment
if (typeof window !== 'undefined') {
  (window as any).LogPresentation = { mapLogToPresentation };
}
